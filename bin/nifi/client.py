...
class NiFiClient:
    ...
    def get_templates(self):
        ...
        if (response.status_code != 200):
            raise Exception(
                "Unable to get current user from {}.".format(end_point))
    ...
    def get_client_id(self):
        ...
        if (response.status_code != 200):
            raise Exception(
                "Unable to get client id from {}.".format(end_point))
    ...
    def get_search(self, query):
        ...
        if (response.status_code != 200):
            print(response)
            raise Exception(
                "Unable to perform search for {} from {}.".format(query, end_point))
        return response.json()

    def get_process_group(self, group_id):
        end_point = "{}flow/process-groups/{}".format(
            self.__base_url, group_id)
        response = requests.get(end_point)
        if (response.status_code != 200):
            raise Exception(
                "Unable to get process group flow from {}.".format(end_point))
        return response.json()
    ...
    def get_pg(self, group_id):
        ...
        if (response.status_code != 200):
            raise Exception(
                "Unable to get process group from {}.".format(end_point))
        return response.json()
    ...
    def get_processor(self, processor_id):
        ...
        if (response.status_code != 200):
            raise Exception(
                "Unable to get processor from {}.".format(end_point))
        return response.json()
    ...
    def delete_process_group(self, group_id, version):
        ...
        if (response.status_code != 200):
            print(response)
            raise Exception(
                "Unable to delete process group {}.".format(group_id))
        return response.json()
    ...
    def create_process_group(self, name, parent_id, pos_x=None, pos_y=None):
        ...
        end_point = "{}process-groups/{}/process-groups".format(
            self.__base_url, parent_id)
    ...
    def add_connection(self, group_id, info):
        """
        """
        end_point = "{}process-groups/{}/connections".format(
            self.__base_url, group_id)
        params = {
            "revision": {
                "clientId": self.__client_id,
    ...
    def instantiate_template(self, group_id, template_id, origin_x=0, origin_y=0):
        end_point = "{}process-groups/{}/template-instance".format(
            self.__base_url, group_id)
        params = {
        ...
        if (response.status_code != 201):
            print(response)
            raise Exception(
                "Unable to instantiate template {}.".format(template_id))
        return response.json()

    def get_pg_vars(self, id):
        end_point = "{base}process-groups/{id}/variable-registry".format(
            base=self.__base_url, id=id)
        self.log.debug('GET {}'.format(end_point))
        response = requests.get(end_point)
        if (response.status_code != 200):
            print(response)
